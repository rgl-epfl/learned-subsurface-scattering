...
public:
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            bool sampled_medium =
                rRec.medium &&
                rRec.medium->sampleDistance(
                    Ray(ray, 0, std::numeric_limits<Float>::infinity()), mRec,
                    rRec.sampler);

            if (!sampled_medium)
                mRec.t = std::numeric_limits<Float>::infinity();

            bool found_intersection =
                scene->rayIntersect(Ray(ray.o, ray.d, ray.mint,
                                        std::min(mRec.t, ray.maxt), ray.time), its);
#else
            scene->rayIntersect(ray, its);
            bool sampled_medium =
                rRec.medium && rRec.medium->sampleDistance(Ray(ray, 0, its.t),
                                                           mRec, rRec.sampler);
            ...
            /* ==================================================================== */
            if (sampled_medium && !found_intersection) {
                /* Sample the integral
...
#if 0 // Don't sample direct illumination inside a medium
                /* ==================================================================== */
