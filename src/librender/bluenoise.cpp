...
void blueNoisePointSet(const Scene *scene, const std::vector<Shape *> &shapes,
        Float radius, std::vector<Point3> &sampledP, std::vector<Vector3> &sampledN, Float &sa, AABB &aabb,
        const void *data) {
    ...
    for (CellMap::const_iterator it = cells.begin();
        ...
        sampledP.push_back(sample.p);
        sampledN.push_back(sample.n);
    }

    SLog(EInfo, "Sampling finished (obtained %i blue noise samples)", (int) sampledP.size());
}
