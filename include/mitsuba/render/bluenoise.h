...
#if !defined(__LIBRENDERBLUENOISE_H)

#define __LIBRENDERBLUENOISE_H
...
//DV Copied from subsurface plugin, slight interface change
 ...
 */
extern MTS_EXPORT_RENDER void blueNoisePointSet(const Scene *scene,
    const std::vector<Shape *> &shapes, Float radius,
    std::vector<Point3> &sampledP, std::vector<Vector3> &sampledN, Float &sa, AABB &aabb,
    const void *data);
